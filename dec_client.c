...
char* srMessage(char* eText, char* key, int socketFD) {
	int tLen = strlen(eText);
    int test1; 
    int test2;
	char* pText;  
	...
	// Send eText to be decrypted
	send(socketFD, eText, tLen * sizeof(char), 0); 
	usleep(S_TIME);	
    ...
    pText = malloc(tLen * sizeof(char));

	// Receive encrypted message from server and store in eText
	recv(socketFD, pText, tLen * sizeof(char), 0);  		           
...
int main(int argc, char *argv[]) {
    ...
    cType = 'd'; //type of client is decryption (confirms correct server connections)
    // Check usage & args
    if (argc < 4) { 
    ...
    keyLengthCheck(strlen(eText), strlen(key), argv[2]);
    ...
    pText = srMessage(eText, key, socketFD);
    printf("%s\n", pText);
